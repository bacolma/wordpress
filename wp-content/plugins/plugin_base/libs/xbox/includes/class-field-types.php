...
class FieldTypes
{
    ...
    private function build_file_item ( $preview_size , $value , $multiple , $attachment_field , $index = NULL )
    {
        ...
        if ( $this->is_image_file( $value ) ) {
            ...
            if ( !empty( $attachment_id ) ) {
                ...
                $item_body = wp_get_attachment_image( $attachment_id , [ $width , $height ] , false , [ 'class' => 'xbox-image xbox-preview-handler' ] );
            }
    ...
    */
    ...
    public function export ( $type = '' )
    {
        ...
        $file_base_name = $import_settings[ 'backup_name' ];
    ...
    */
    public function image_selector ( $type = '' )
    {
        ...
        foreach ( $items as $key => $image ) {
            ...
            $return .= $this->build_input( $options[ 'like_checkbox' ] ? 'checkbox' : 'radio' , $key , [ 'data-image' => $image ] );
    ...
    public function number ( $type = '' )
    {
        ...
        if ( !Functions::is_empty( $attributes ) ) {
            foreach ( $attributes as $attr => $val ) {
                if ( in_array( $attr , [ 'min' , 'max' , 'step' , 'precision' ] ) ) {
        ...
        $return = $this->build_input( 'text' , '' , [ 'data-default-unit' => $options[ 'unit' ] ] , 'esc_attr' , [ 'min' , 'max' , 'step' , 'precision' ] );
    ...
    */
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public function build_input ( $type = 'text' , $value = '' , $attributes = [] , $escaping_function = 'esc_attr' , $exclude_attributes = [] )
    {
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public function build_select ( $type = 'select' , $value = '' , $attributes = [] , $escaping_function = 'esc_attr' )
    {
        $items = $this->field->arg( 'items' );
    ...
    */
    public function is_image_file ( $file_path = '' )
    {
        ...
        if ( $extension && in_array( $extension , [ 'png' , 'jpg' , 'jpeg' , 'gif' , 'ico' ] ) ) {
            return true;
        }
        return false;
    }
    ...
    */
    ...
    {
        $extension = Functions::get_file_extension( $file_path );
        if ( $extension && in_array( $extension , [ 'mp4' , 'webm' , 'ogv' , 'ogg' , 'vp8' ] ) ) {
            return true;
        }
        return false;
    }
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public function join_attributes ( $attrs = [] )
    {
        $attributes = '';
